...
function startCountdown() {
  ...
  if (releaseDateLabel) {
    releaseDateLabel.textContent = formatted + " WIB";
  }
  ...
  function tick() {
    ...
    if (diff <= 0) {
      // Waktu habis, hentikan countdown dan tampilkan login
      if (countdownInterval) {
        clearInterval(countdownInterval);
        countdownInterval = null;
      }
      if (phaseCountdown) phaseCountdown.classList.add('hidden');
      if (phaseLogin) phaseLogin.classList.remove('hidden');
      return;
    }
...
// ============================================================
// SHOW PHASE (Countdown / Login / Result)
// ============================================================
function showPhase(phaseName) {
  const phases = ['phaseCountdown', 'phaseLogin', 'phaseResult'];
...
// ============================================================
// INITIALIZATION - Tentukan Fase Awal
// ============================================================
function init() {
  ...
  if (sekarang >= target) {
    // Sudah melewati waktu pembukaan → tampilkan login
